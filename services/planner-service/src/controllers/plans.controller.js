...
async function getPlan(req, res) {
  ...
  try {
    const [plans] = await db.query('SELECT * FROM plans WHERE id = ? AND owner_id = ?', [
      id,
      userId,
    ]);
    if (plans.length === 0) {
...
async function deletePlan(req, res) {
  ...
  try {
    const [plans] = await db.query('SELECT owner_id FROM plans WHERE id = ?', [id]);
    if (plans.length === 0) {
      return res.status(404).json({ error: 'Plan not found' });
    }
...
async function addPlanItem(req, res) {
  ...
  const { idea_id, custom_title, custom_description, start_time, end_time, location } = req.body;

  try {
    // check ownership
    const [plans] = await db.query('SELECT owner_id FROM plans WHERE id = ?', [planId]);
    if (plans.length === 0) {
      return res.status(404).json({ error: 'Plan not found' });
    }
    if (plans[0].owner_id !== userId) {
    ...
    const [rows] = await db.query('SELECT * FROM plan_items WHERE id = ?', [result.insertId]);
...
async function deletePlanItem(req, res) {
  ...
  try {
    ...
    await db.query('DELETE FROM plan_items WHERE id = ? AND plan_id = ?', [itemId, planId]);
